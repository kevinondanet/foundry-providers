...
- Order and gating of every emission: run start → task start → per sample `init` (first attempt, before sandboxes) →
  ...
  task end (after the log is written) → run end (once, with the exception that escaped the run).
