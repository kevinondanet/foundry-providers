...
- `adaptive_connections` lives on `Model` / `EvalOptions` (`AdaptiveConnections`), not on `GenerateConfig` (whose `AdaptiveConnections` value, added by model-extras, is honored through `AdaptiveConnections.FromConfigValue` when the `Model` setting is unset),
