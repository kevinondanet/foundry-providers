...
| `util/_limit.py` `cost_limit`, `record_model_cost`, `check_cost_limit`, `_CostLimit` | `Context/Limits.cs` (`CostLimit`, `CostUsage`, `RecordModelCost`, `CheckCostLimit`) | Trips with `LimitExceededException("cost")` and the Python message (`Cost limit exceeded. value: $0.0070; limit: $0.0050`) after recording a `sample_limit` event of type `cost` on the transcript, strict `>`, negative limits rejected, `Suspend()` stops enforcement like the other limits. |
...
Exactly where Python records it. `record_and_check_model_usage` sets `usage.total_cost` on the output's usage object, which the returned output and the `ModelEvent` share; here `Model.GenerateAsync` prices the output (`ModelCosts.PriceOutput`) before the event is recorded, so `ModelEvent.Output.Usage.TotalCost`, the returned `ModelOutput.Usage.TotalCost`, `EvalSample.ModelUsage[model].TotalCost` (via `Limits.AddUsage`, which sums `total_cost` with the `ModelUsage` operator) and `EvalStats.ModelUsage[model].TotalCost` (the runner's aggregate) all carry it, serialised as `total_cost`. Python's `ModelEvent`, `EvalSample` and `EvalStats` have no separate cost field, so none was added. `Model.GenerateAsync` follows the order of `record_and_check_model_usage`: `Limits.RecordUsage` and `TokenLimit.RecordModelUsage` record the usage on the sample totals and the scoped token-limit tree, the token limits are checked root first, and only then is the cost recorded (`Limits.RecordModelCost`) and checked (`Limits.CheckCostLimit`). A call that trips both a token limit and the cost limit therefore raises the token limit with its tokens counted in the tree, and leaves `Limits.CostUsage` unrecorded for that call while the usage dictionaries still carry the cost. `Limits.AddUsage` runs the sample-level steps in that order for callers without a scoped tree.
