...
- `Model.GenerateAsync` hooks: `MessageLimit.CheckMessageLimit(count, raiseForEqual: true)` before the call; `TokenLimit.RecordModelUsage` + `CheckTokenLimit` and `TurnLimit.RecordTurn()` after a successful one; retry waits reported as waiting time. `TaskState.MessageLimit`/`TokenLimit` write through to the sample-level scopes once the runner attaches them (`state.MessageLimit = 50` retunes the live limit, as in Python).
- `SampleLimitEvent(Type, Message, Limit)` is recorded on every trip — scoped limits, the flat `Limits` checks, the runner's time and working deadlines — and round-trips through the JSON log as `sample_limit`.
