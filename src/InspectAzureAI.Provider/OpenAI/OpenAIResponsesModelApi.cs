...
public sealed class OpenAIResponsesModelApi : IModelApi, IDisposable
{
    ...
    public string QualifiedModelName => "openai/azure/" + DeploymentName;
    public bool IsFoundry => true;
    public string ConnectionKey() => $"{BaseUrl}:{ModelName}";
    public bool ApplyRedactedReasoningTokensToInput() => true;
    ...
    /// </summary>
    public JsonObject BuildRequest(IReadOnlyList<ChatMessage> input, IReadOnlyList<ToolInfo> tools, ToolChoice toolChoice, GenerateConfig config, bool streaming) => new ResponsesProtocol(DeploymentName, ModelArgs).BuildRequest(input, tools, toolChoice, config, streaming);
    public static JsonObject ReasoningParams(string deploymentName, GenerateConfig config) => ResponsesProtocol.ReasoningParams(deploymentName, config);
    public static bool ReasoningEnabled(string deploymentName, GenerateConfig config) => ResponsesProtocol.ReasoningEnabled(deploymentName, config);
    public static string UnsupportedParamWarning(string param) => ResponsesProtocol.UnsupportedParamWarning(param);
