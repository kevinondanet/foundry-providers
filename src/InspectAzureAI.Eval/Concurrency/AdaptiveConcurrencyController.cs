...
public sealed class AdaptiveConcurrencyController : IConcurrencySemaphore
{
    ...
    private readonly List<ConnectionLimitChange> _history = [];
    ...
    public IReadOnlyList<ConnectionLimitChange> History
    {
        get
        {
            lock (_sync)
    ...
    private void SetLimit(int target, LimitChangeReason reason)
    {
        ...
        _history.Add(new ConnectionLimitChange(_time.GetUtcNow().ToUnixTimeMilliseconds() / 1000.0, Name, old, target, reason));
