...
def slugify(text: str, *, max_length: int | None = None) -> str:
    ...
    Accents are stripped, runs of non-alphanumerics collapse to one hyphen, and leading or
    trailing hyphens are removed. When ``max_length`` is given the slug is cut at a word boundary
    so the result never exceeds it. Raises ``TypeError`` for non-string input.
    """
