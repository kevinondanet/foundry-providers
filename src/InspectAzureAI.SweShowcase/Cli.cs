...
internal static class Cli
{
    public const string Help = """
        ...
        commands:
          ...
          run                       run one task with one agent and write a JSON eval log
          show <log.json>           print the scores and a per-sample summary of a log
          --help                    this text
        ...
        run options:
          ...
          --log-dir DIR             where the JSON eval log is written (default logs)
    ...
    public static int Show(List<string> rest)
    {
        if (rest.Count != 1)
        {
            throw new UsageError("show expects exactly one argument: the path of an eval log (.json)");
        }
        ...
        try
        {
            log = EvalLogWriter.Read(path);
        }
        catch (JsonException ex)
        {
            throw new UsageError($"{path} is not an eval log written by this showcase: {ex.Message}");
        }
        ...
        foreach (var sample in log.Samples ?? [])
        {
            ...
            var line = $"  {sample.Id} (epoch {sample.Epoch}): {scores} | {tokens} tokens | {seconds}s | {modelCalls} model calls, {toolCalls} tool calls";
    ...
    private static async Task<int> RunEvalAsync(RunOptions options, List<string> rest, CancellationToken cancellationToken)
    {
        ...
        var evalOptions = new EvalOptions
        {
            Model = model,
            Limit = options.Limit,
            SampleIds = options.SampleIds.Count > 0 ? options.SampleIds : null,
            Epochs = options.Epochs,
            MaxSamples = options.MaxSamples,
            LogDir = options.LogDir,
            Cleanup = options.Cleanup,
            Reporter = new ConsoleEvalReporter(Console.Out),
        };
