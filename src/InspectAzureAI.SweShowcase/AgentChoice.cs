...
/// <summary>
/// The agents the showcase can run and how each becomes a task solver: the two Inspect SWE agents and the Agent
/// Framework agent go through <c>as_solver</c> (<see cref="Agents.AsSolver"/>), the basic agent is Inspect's
/// <c>basic_agent</c> with the sandbox <c>bash</c> tool.
/// </summary>
internal static class AgentChoice
{
    ...
    public static readonly IReadOnlyList<string> Names = [MiniSweName, ClaudeCodeName, BasicName, MafName];
    ...
    /// Code, the Inspect chat client for the Agent Framework agent); <paramref name="compaction"/> applies to the two
    /// native loops only — the Claude Code CLI and Agent Framework manage their own context, so asking for it there
    /// is a usage error (see <see cref="RejectCompaction"/>). Approval policies are not passed here: the runner
    /// installs the eval's policies ambiently and all four agents pick them up.
    /// </summary>
    ...
    /// <summary>Compaction is a property of the native loops; the Claude Code CLI and Agent Framework manage their own context windows.</summary>
